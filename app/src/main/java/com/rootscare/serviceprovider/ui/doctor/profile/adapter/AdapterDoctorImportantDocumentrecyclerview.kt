...
class AdapterDoctorImportantDocumentrecyclerview(val qualificationDataList: ArrayList<QualificationDataItem>?, internal var context: Context
) : RecyclerView.Adapter<AdapterDoctorImportantDocumentrecyclerview.ViewHolder>() {
...
//        return trainerList!!.size
    ...
    inner class ViewHolder(internal var itemVie: ItemDoctorImportantDocumentRecyclerviewBinding) : RecyclerView.ViewHolder(itemVie.root) {
        ...
        fun onBind(pos: Int) {
            ...
            if (qualificationDataList?.get(pos)?.qualificationCertificate != null
                && !TextUtils.isEmpty(qualificationDataList[pos].qualificationCertificate?.trim())
                && !qualificationDataList[pos].qualificationCertificate?.contains(".pdf")!!
            ) {
                val circularProgressDrawable = CircularProgressDrawable(context)
                circularProgressDrawable.strokeWidth = 5f
                circularProgressDrawable.centerRadius = 30f
                circularProgressDrawable.setColorSchemeColors(R.color.green_light_1, R.color.green)
                ...
                val imageUrl =
                    BaseMediaUrls.USERIMAGE.url + qualificationDataList.get(pos).qualificationCertificate
