...
import com.rootscare.serviceprovider.ui.physiotherapy.physiotherapyMyAppointment.subfragment.upcomingappointment.adapter.AdapterPhysiotherapistUpcomingAppointment
import java.util.*


class FragmentUpcomingAppointmentForPhysiotherapist :
    BaseFragment<FragmentDoctorUpcomingAppointmentBinding, FragmentUpcomingAppointmentForPhysiotherapistViewModel>(),
    FragmentUpcomingAppointmentForPhysiotherapistNavigator {
    private var lastPosition: Int? = null
    private var contactListAdapter: AdapterPhysiotherapistUpcomingAppointment? = null

    private var fragmentDoctorUpcomingAppointmentBinding: FragmentDoctorUpcomingAppointmentBinding? = null
    private var fragmentUpcomingAppointmentViewModel: FragmentUpcomingAppointmentForPhysiotherapistViewModel? = null
    ...
    override val viewModel: FragmentUpcomingAppointmentForPhysiotherapistViewModel
        get() {
            fragmentUpcomingAppointmentViewModel = ViewModelProviders.of(this).get(
                FragmentUpcomingAppointmentForPhysiotherapistViewModel::class.java
            )
            return fragmentUpcomingAppointmentViewModel as FragmentUpcomingAppointmentForPhysiotherapistViewModel
        }

    companion object {
        fun newInstance(): FragmentUpcomingAppointmentForPhysiotherapist {
            val args = Bundle()
            val fragment = FragmentUpcomingAppointmentForPhysiotherapist()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fragmentUpcomingAppointmentViewModel!!.navigator = this
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (isNetworkConnected) {
            ...
            getDoctorUpcomingAppointmentRequest.userId = fragmentUpcomingAppointmentViewModel?.appSharedPref?.loginUserId
//            getDoctorUpcomingAppointmentRequest.userId="18"
            fragmentUpcomingAppointmentViewModel!!.apiNurseUpComingAppointmentList(getDoctorUpcomingAppointmentRequest)
        } else {
            ...
            val dpd = DatePickerDialog(this.requireActivity(), { _, yearInScope, monthOfYear, dayOfMonth ->
                ...
                if (fragmentDoctorUpcomingAppointmentBinding?.txtUpcomingDate?.text?.toString() != null && !fragmentDoctorUpcomingAppointmentBinding?.txtUpcomingDate?.text?.toString()
                    ...
                    if (isNetworkConnected) {
                        ...
                        filterAppointmentRequest.userId = fragmentUpcomingAppointmentViewModel?.appSharedPref?.loginUserId
//                        filterAppointmentRequest.userId="18"
                        ...
                        fragmentUpcomingAppointmentViewModel!!.apiFilterNurseUpComingAppointmentList(filterAppointmentRequest)
                    } else {
                        Toast.makeText(activity, "Please check your network connection.", Toast.LENGTH_SHORT).show()
                    }
                }
...
//        val contactListAdapter = AdapterHospitalRecyclerviw(trainerList,context!!)
        contactListAdapter = AdapterPhysiotherapistUpcomingAppointment(upcomingAppointmentList, requireContext())
                ...
                CommonDialog.showDialog(activity!!, object :
                    ...
                    override fun onConfirm() {
                        if (isNetworkConnected) {
                            ...
                            updateAppointmentRequest.id = contactListAdapter?.upcomingAppointmentList!![id.toInt()]?.id
                            updateAppointmentRequest.acceptanceStatus = text
                            fragmentUpcomingAppointmentViewModel!!.apiUpdateNurseAppointmentRequest(
