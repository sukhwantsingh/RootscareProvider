...
class FragmentTodaysAppointmentForBabySitterViewModel  : BaseViewModel<FragmentTodaysAppointmentForBabySitterNavigator>() {
    fun apiNurseTodayAppointmentList(getDoctorUpcomingAppointmentRequest: GetDoctorUpcommingAppointmentRequest) {
//        val body = RequestBody.create(MediaType.parse("application/json"), "")
        val disposable = apiServiceWithGsonFactory.apiBabySitterTodayAppointmentList(getDoctorUpcomingAppointmentRequest)
    ...
    fun apiHitForMarkAsCompleteForNurse(getDoctorUpcomingAppointmentRequest: CommonUserIdRequest, position:Int) {
        val disposable = apiServiceWithGsonFactory.apiHitForMarkAsCompleteForBabySitter(getDoctorUpcomingAppointmentRequest)
            .subscribeOn(_scheduler_io)
            .observeOn(_scheduler_ui)
            .subscribe({ response ->
                if (response != null) {
