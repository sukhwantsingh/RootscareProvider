...
import com.rootscare.serviceprovider.ui.physiotherapy.physiotherapyMyAppointment.subfragment.newappointment.adapter.AdapterRequestedAppointmentListForPhysiotherapist
import java.util.*

class FragmentRequestedAppointmentForPhysiotherapist : BaseFragment<FragmentDoctorRequestedAppointmentBinding,
        FragmentRequestedAppointmentForPhysiotherapistViewModel>(), FragmentRequestedAppointmentForPhysiotherapistNavigator {
    private var lastPosition: Int? = null
    private var contactListAdapter: AdapterRequestedAppointmentListForPhysiotherapist? = null
    ...
    private var fragmentRequestedAppointmentViewModel: FragmentRequestedAppointmentForPhysiotherapistViewModel? = null
    override val bindingVariable: Int
        get() = BR.viewModel
    override val layoutId: Int
        get() = R.layout.fragment_doctor_requested_appointment
    override val viewModel: FragmentRequestedAppointmentForPhysiotherapistViewModel
        get() {
            fragmentRequestedAppointmentViewModel = ViewModelProviders.of(this).get(
                FragmentRequestedAppointmentForPhysiotherapistViewModel::class.java
            )
            return fragmentRequestedAppointmentViewModel as FragmentRequestedAppointmentForPhysiotherapistViewModel
        }

    companion object {
        fun newInstance(): FragmentRequestedAppointmentForPhysiotherapist {
            val args = Bundle()
            val fragment = FragmentRequestedAppointmentForPhysiotherapist()
...
//            getDoctorUpcomingAppointmentRequest.userId="18"
            fragmentRequestedAppointmentViewModel!!.apiNurseAppointmentRequestList(getDoctorUpcomingAppointmentRequest)
        } else {
...
//        val contactListAdapter = AdapterHospitalRecyclerviw(trainerList,context!!)
        contactListAdapter = AdapterRequestedAppointmentListForPhysiotherapist(requestedappointmentList, requireContext())
    ...
    override fun successGetDoctorRequestAppointmentResponse(getDoctorRequestAppointmentResponse: GetDoctorRequestAppointmentResponse?) {
        ...
        if (getDoctorRequestAppointmentResponse?.code.equals("200")) {
            if (getDoctorRequestAppointmentResponse?.result != null && getDoctorRequestAppointmentResponse.result.size > 0) {
                ...
                fragmentDoctorRequestedAppointmentBinding?.tvNoDate?.text = "Physiotherapy Appointment Request Not Found."
            }
