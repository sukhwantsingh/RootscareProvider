...
class ManageDocLabViewModel : BaseViewModel<ManageDocLabNavigator>() {

    fun getHospitalDocList(reqbody: RequestBody?) {
        val disposable = apiServiceWithGsonFactory.getHospitalDocList(reqbody)
            .subscribeOn(_scheduler_io)
            .observeOn(_scheduler_ui)
            .subscribe({ response ->
                if (response != null) {
                  navigator.onSuccessDocResponse(response)
                }
            }, { throwable ->
                run {
