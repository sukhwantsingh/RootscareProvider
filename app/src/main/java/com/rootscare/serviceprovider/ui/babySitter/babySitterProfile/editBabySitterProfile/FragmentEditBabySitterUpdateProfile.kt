...
import com.rootscare.serviceprovider.ui.babySitter.babySitterProfile.editBabySitterProfile.adapter.BabySitterCertificateListAdapter
...
class FragmentEditBabySitterUpdateProfile :
    BaseFragment<FragmentDoctorEditProfileBinding, FragmentEditBabySitterUpdateProfileViewModel>(),
    FragmentEditBabySitterUpdateProfileNavigator {

    private var imageSelectionModel: AddlabTestImageSelectionModel? = null
    private val TAG = "FragmentEditDoctorProfile"
    private var certificateListAdapter: BabySitterCertificateListAdapter? = null
...
//    private var dobToSubmit: String? = null
    ...
    private var fragmentEditDoctorProfileViewModel: FragmentEditBabySitterUpdateProfileViewModel? = null
    override val bindingVariable: Int
        get() = BR.viewModel
    override val layoutId: Int
        get() = R.layout.fragment_doctor_edit_profile
    override val viewModel: FragmentEditBabySitterUpdateProfileViewModel
        get() {
            fragmentEditDoctorProfileViewModel = ViewModelProviders.of(this).get(
                FragmentEditBabySitterUpdateProfileViewModel::class.java
            )
            return fragmentEditDoctorProfileViewModel as FragmentEditBabySitterUpdateProfileViewModel
        }

    companion object {
        fun newInstance(userType: String): FragmentEditBabySitterUpdateProfile {
            val args = Bundle()
            args.putString("userType", userType)
            val fragment = FragmentEditBabySitterUpdateProfile()
        ...
        with(fragmentDoctorEditProfileBinding!!) {
            if (fragmentEditDoctorProfileViewModel?.appSharedPref?.loginUserId != null && checkValidationForRegStepOne()) {
                ...
                val fees = ediitextFees.text?.trim().toString()
                    .toRequestBody("multipart/form-data".toMediaTypeOrNull())
                val department = departmentId.trim().toString()
                    .toRequestBody("multipart/form-data".toMediaTypeOrNull())
...
//            activityLoginBinding?.edtPassword?.setError("Please enter Password")
    ...
    private fun addCertificatePortionsSetUp() {
        with(fragmentDoctorEditProfileBinding!!) {
            ...
            certificateListAdapter = BabySitterCertificateListAdapter(requireActivity())
