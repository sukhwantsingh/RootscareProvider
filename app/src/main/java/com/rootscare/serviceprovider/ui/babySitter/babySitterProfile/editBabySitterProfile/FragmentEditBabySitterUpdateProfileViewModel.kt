...
class FragmentEditBabySitterUpdateProfileViewModel : BaseViewModel<FragmentEditBabySitterUpdateProfileNavigator>() {
...
//    }
    ...
    fun apiHitForUpdateProfileWithProfileAndCertificationImage(
        ...
        val disposable = apiServiceWithGsonFactory.apiHitUpdateProfileWithImageAndCertificate(
            user_id, first_name, last_name, email, mobile_number, dob, gender, qualification, passing_year, institute, description,
            experience, available_time, fees, department, startTime, endTime, image, certificate
        )
            .subscribeOn(_scheduler_io)
            .observeOn(_scheduler_ui)
            .subscribe({ response ->
