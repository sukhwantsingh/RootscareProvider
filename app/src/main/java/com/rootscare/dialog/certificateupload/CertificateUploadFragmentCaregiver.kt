...
class CertificateUploadFragmentCaregiver :
    BaseDialogFragment<CertificateUploadDialogBinding, CertificateViewModel>(),
    ...
    interface PassDataCallBack{
        fun onPassData(data: com.rootscare.data.model.response.caregiver.profileresponse.QualificationDataItem)
    }
    ...
    companion object {
        fun newInstance(): CertificateUploadFragmentCaregiver {
            val args = Bundle()
            val fragment = CertificateUploadFragmentCaregiver()
        ...
        with(fragmentLayoutBinding!!) {
            ...
            tvSubmit.setOnClickListener {
                if (checkValidationForRegStepOne()){
                    val model = com.rootscare.data.model.response.caregiver.profileresponse.QualificationDataItem()
        ...
        if (requestCode == PICKFILE_RESULT_CODE) {
            if (resultCode == -1) {
                ...
                try {
                    ...
                    fragmentLayoutBinding?.textViewCertificate?.text = getFileName(activity!!, fileUri!!)
                } catch (e: IOException) {
                    e.printStackTrace()
                }
            }
